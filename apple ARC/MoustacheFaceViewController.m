...
#pragma mark - UIImage(RotationMethods)
...
#pragma mark - MoustacheFaceViewController (InternalMethods)

@interface MoustacheFaceViewController (InternalMethods)
- (void)setupAVCapture;
                                     ...
                                     frontFacing:(BOOL)isFrontFacing
{
    ...
    UIImage *rotateMoustacheImage = [moustache imageRotatedByDegrees:rotationDegrees];
	
    // features found by the face detector
	for ( CIFaceFeature *ff in features ) {
		CGRect faceRect = [ff bounds];
		CGContextDrawImage(bitmapContext, faceRect, [rotatedSquareImage CGImage]);
        CGContextDrawImage(bitmapContext, faceRect, [rotateMoustacheImage CGImage]);
	}
...
- (void)drawFaceBoxesForFeatures:(NSArray *)features forVideoBox:(CGRect)clap orientation:(UIDeviceOrientation)orientation
{
	...
	CGRect previewBox = [MoustacheFaceViewController videoPreviewBoxForGravity:gravity 
	...
	for ( CIFaceFeature *ff in features ) {
		...
		while ( !featureLayer && (currentSublayer < sublayersCount) ) {
			...
			if ( [[currentLayer name] isEqualToString:@"FaceLayer"] ) {
                ...
                moustacheLayer = [currentLayer.sublayers objectAtIndex:0];
        ...
        if ( !featureLayer ) {
            moustacheLayer = [CALayer new];
            [moustacheLayer setContents:(id)[moustache CGImage]];
            [moustacheLayer setName:@"MoustacheLayer"];
            featureLayer = [CALayer new];
            //[featureLayer setContents:(id)[square CGImage]]; //just dont show the red square
            [featureLayer setName:@"FaceLayer"];
            
            [featureLayer addSublayer:moustacheLayer];
            
            [previewLayer addSublayer:featureLayer];
        }
        
        float moustacheResizeRatio = faceRect.size.width/moustache.size.width;
        float moustacheHeight = moustache.size.height * moustacheResizeRatio;
        moustacheLayer.frame = CGRectMake(0 , faceRect.size.height * .8 - moustacheHeight, faceRect.size.width, moustacheHeight);
...
- (void)viewDidLoad
{
    ...
    moustache = [UIImage imageNamed:@"moustache"];
